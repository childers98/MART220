...
//     //preload the images and the font here from the assets folder
    // electric = src="assets/electric.png");
...
function draw() {
    ...
    rect2X += rect2Speed;
    if (rect2X >= 700 || rect2X <= 0) {
        rect2Speed *= -1;
    }
    rect2Y += rect2Speed
    if (rect2Y >= 700 || rect2Y <= 0) {
        rect2Speed *= -1;
    }
    ...
    textSize(22);
    text("Paige Childers", 525, 470);
    text("Bold & Beautiful Shapes", 50, 50);
    // add the time passed text
    text("Time Passed: " + timerText, 250, 70);
    if (timerValue >= 5){
